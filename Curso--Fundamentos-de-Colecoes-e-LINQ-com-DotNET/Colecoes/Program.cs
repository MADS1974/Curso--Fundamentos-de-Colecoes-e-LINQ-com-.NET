...
namespace Colecoes
{
  class Program
  {
    static void Main(string[] args)
    {
      ...
      List<string> estados = new List<string> { "Rio de Janeiro", "São Paulo", "Bahia" };
      string[] estadosArray = new string[2] { "Alagoas", "Paraiba" };

      System.Console.WriteLine($"Quantos e quais estados Brasileiros já conheci?: {estados.Count}");

      opLista.ImprimirListaString(estados);

      System.Console.WriteLine("Qual estados estão na lista do seu próximo TOUR?");
      estados.Remove("São Paulo");

      estados.AddRange(estadosArray);
      estados.Insert(1, "Rio Grande do Norte");
      ...
      fila.Enqueue("Da Silva");

      System.Console.WriteLine($"Pessoas na fila: {fila.Count}");

      while (fila.Count > 0)
      {
        System.Console.WriteLine($"É a Vez de: {fila.Peek()}");
      ...
      estadosDicionario.Add("RJ", "Rio de Janeiro");
      ...
      //var teste = estadosDicionario["AL"];
      ...
      // ********  Operações com LINQ  ********
      int[] arrayNumeros = new int[10] { 200, 1, 6, 0, 12, 15, 21, 21, 6, 200 };
